...
```
```
Dueling DQN Lightweight Dueling DQN Vanilla DQN
```
```
Input
Normalization
...
```
```
Feature Layers Linear(3→128) + ReLU Linear(3→64) →
Linear(64→12) + ReLU
```
```
Linear(3→64) + ReLU
```
```
Value Stream Linear(128→128) → ReLU →
Linear→
```
```
Linear(12→1) None (not dueling)
```
```
Advantage
Stream
...
```
```
Target Usage Soft update every 50
episodes
```
```
Same Same
